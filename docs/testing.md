...
- focused-book and all-library retrieval boundaries
- no-result behavior that avoids answer generation
...
3. Test the OpenAI client with a mock HTTP server.
...
Retrieval tests should verify that a focused book never retries without its book ID, **All books** searches the complete indexed library, and empty retrieval returns a not-found response without any web request.
